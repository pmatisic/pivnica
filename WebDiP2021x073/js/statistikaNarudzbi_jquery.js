...
function generirajFiltere() {
    filter = '<label for="brojnar" class="labele">Broj narudžbe: </label>';
    filter += '<input id="brojnar" onkeyup="dohvatiStatistiku();" name="brojnar" class="polja" type="number"><br>';
    filter += '<label for="zem" class="labele">Zemlja porijekla piva: </label>';
    filter += '<input id="zem" onkeyup="dohvatiStatistiku();" name="zem" class="polja" type="text"><br>';
...
function dohvatiStatistiku() {
    ...
    setTimeout(function () {
        ...
        tablica += '<th>ID piva <a href="#" onclick="postaviSortiranje(00);">S</a> <a href="#" onclick="postaviSortiranje(01);">U</a></th>';
        tablica += '<th>Broj narudžbe <a href="#" onclick="postaviSortiranje(10);">S</a> <a href="#" onclick="postaviSortiranje(11);">U</a></th>';
        tablica += '<th>Datum narudžbe </th>';
        tablica += '<th>Cijena </th>';
        tablica += '<th>Volumen </th>';
        tablica += '<th>Zemlja porijekla </th>';
        ...
        for (i = velicinaStranicanja * trenutnaStranica; i < gornjaGranica; i++) {
            tablica += '<tr>';
            tablica += '<td>' + podatak[i].id_pivo + '</td>';
            tablica += '<td>' + podatak[i].broj_narudzbe + '</td>';
            tablica += '<td>' + podatak[i].vrijeme_postavljanja + '</td>';
            tablica += '<td>' + podatak[i].cijena + '</td>';
            tablica += '<td>' + podatak[i].volumen + '</td>';
