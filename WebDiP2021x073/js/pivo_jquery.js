...
function generirajFiltere() {
    filter = '<label for="cijen" class="labele">Cijena: </label>';
    filter += '<input id="cijen" onkeyup="generirajSadrzaj();" name="cijen" class="polja" type="number" step=".01"><br>';
    filter += '<label for="ime" class="labele">Naziv piva: </label>';
...
function generirajSadrzaj() {
    ...
    setTimeout(function () {
        ...
        tablica += '<th>Naziv <a href="#" onclick="postaviSortiranje(00);">S</a> <a href="#" onclick="postaviSortiranje(01);">U</a></th>';
        tablica += '<th>Cijena </th>';
        tablica += '<th>Opis </th>';
        tablica += '<th>Volumen <a href="#" onclick="postaviSortiranje(00);">S</a> <a href="#" onclick="postaviSortiranje(01);">U</a></th>';
        tablica += '<th>Rok trajanja </th>';
        tablica += '<th>Slika </th>';
        ...
        for (i = velicinaStranicanja * trenutnaStranica; i < gornjaGranica; i++) {
            ...
            tablica += '<td>' + podatak[i].opis + '</td>';
            tablica += '<td>' + podatak[i].volumen + '</td>';
            tablica += '<td>' + podatak[i].rok_trajanja + '</td>';
            tablica += '<td>' + podatak[i].slika + '</td>';
            tablica += '</tr>';
